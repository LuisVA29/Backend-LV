...
class UserManagerService {
  ...
  async changeUserPassword(token, newPass) {
    try {
      ...
      return await userModels
        .findOneAndUpdate({ resetLink: token }, { password: newHashedPass })
        .then((res) => {
          return res;
        })
        .catch((err) => {
  ...
  async setResetLink(uid, token) {
    try {
      return await userModels
        .findByIdAndUpdate(uid, { resetLink: token })
        .then((res) => {
          return res;
        })
        .catch((err) => {
  ...
  async setDocument(uid, type, path) {
    try {
      ...
      if (docExist.length === 0) {
        const documentAdd = await userModels
          .findOneAndUpdate(
            { _id: uid },
        ...
        const documentReplace = await userModels
          .findOneAndUpdate(
            { _id: uid, "documents.name": type },
            {
              $set: {
